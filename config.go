...
// server config
const (
	defaultServerGatewayPort = "8099" // default port
)

// server mode
const (
	DebugMode   = "debug"   // gin.DebugMode
	ReleaseMode = "release" // gin.ReleaseMode
	TestMode    = "test"    // gin.TestMode
)

// server env
const (
	envKeyServerGatewayHost   = "BhServerGatewayHost"   // server host
	envKeyServerGatewayPort   = "BhServerGatewayPort"   // server port
	envKeyServerMode          = "BhServerMode"          // server mode
...
var DefaultConfigFn = func() {
	...
	// mode
	if mode := strings.TrimSpace(os.Getenv(envKeyServerMode)); len(mode) > 0 {
		cfg.ServerMode = mode
	} else {
		cfg.ServerMode = gin.ReleaseMode
	}

	// host
	if host := strings.TrimSpace(os.Getenv(envKeyServerGatewayHost)); len(host) > 0 {
		cfg.ServerHost = host
	} else {
	...
	// port
	if port := strings.TrimSpace(os.Getenv(envKeyServerGatewayPort)); len(port) > 0 {
		cfg.ServerPort = strings.TrimPrefix(port, ":")
	} else {
		cfg.ServerPort = defaultServerGatewayPort
	}
